...
type BuildPackLister interface {
	...
	List(selector labels.Selector) (ret []*v4beta1.BuildPack, err error)
...
// List lists all BuildPacks in the indexer.
func (s *buildPackLister) List(selector labels.Selector) (ret []*v4beta1.BuildPack, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v4beta1.BuildPack))
	})
	return ret, err
}
...
type BuildPackNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v4beta1.BuildPack, error)
...
// List lists all BuildPacks in the indexer for a given namespace.
func (s buildPackNamespaceLister) List(selector labels.Selector) (ret []*v4beta1.BuildPack, err error) {
...
// Get retrieves the BuildPack from the indexer for a given namespace and name.
func (s buildPackNamespaceLister) Get(name string) (*v4beta1.BuildPack, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v4beta1.Resource("buildpack"), name)
	}
	return obj.(*v4beta1.BuildPack), nil
}
