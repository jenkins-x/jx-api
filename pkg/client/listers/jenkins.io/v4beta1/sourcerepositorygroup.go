...
type SourceRepositoryGroupLister interface {
	...
	List(selector labels.Selector) (ret []*v4beta1.SourceRepositoryGroup, err error)
...
// List lists all SourceRepositoryGroups in the indexer.
func (s *sourceRepositoryGroupLister) List(selector labels.Selector) (ret []*v4beta1.SourceRepositoryGroup, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v4beta1.SourceRepositoryGroup))
	})
	return ret, err
}
...
type SourceRepositoryGroupNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v4beta1.SourceRepositoryGroup, error)
...
// List lists all SourceRepositoryGroups in the indexer for a given namespace.
func (s sourceRepositoryGroupNamespaceLister) List(selector labels.Selector) (ret []*v4beta1.SourceRepositoryGroup, err error) {
...
// Get retrieves the SourceRepositoryGroup from the indexer for a given namespace and name.
func (s sourceRepositoryGroupNamespaceLister) Get(name string) (*v4beta1.SourceRepositoryGroup, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v4beta1.Resource("sourcerepositorygroup"), name)
	}
	return obj.(*v4beta1.SourceRepositoryGroup), nil
}
