...
type EnvironmentLister interface {
	...
	List(selector labels.Selector) (ret []*v4beta1.Environment, err error)
...
// List lists all Environments in the indexer.
func (s *environmentLister) List(selector labels.Selector) (ret []*v4beta1.Environment, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v4beta1.Environment))
	})
	return ret, err
}
...
type EnvironmentNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v4beta1.Environment, error)
...
// List lists all Environments in the indexer for a given namespace.
func (s environmentNamespaceLister) List(selector labels.Selector) (ret []*v4beta1.Environment, err error) {
...
// Get retrieves the Environment from the indexer for a given namespace and name.
func (s environmentNamespaceLister) Get(name string) (*v4beta1.Environment, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v4beta1.Resource("environment"), name)
	}
	return obj.(*v4beta1.Environment), nil
}
