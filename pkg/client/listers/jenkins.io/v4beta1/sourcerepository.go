...
type SourceRepositoryLister interface {
	...
	List(selector labels.Selector) (ret []*v4beta1.SourceRepository, err error)
...
// List lists all SourceRepositories in the indexer.
func (s *sourceRepositoryLister) List(selector labels.Selector) (ret []*v4beta1.SourceRepository, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v4beta1.SourceRepository))
	})
	return ret, err
}
...
type SourceRepositoryNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v4beta1.SourceRepository, error)
...
// List lists all SourceRepositories in the indexer for a given namespace.
func (s sourceRepositoryNamespaceLister) List(selector labels.Selector) (ret []*v4beta1.SourceRepository, err error) {
...
// Get retrieves the SourceRepository from the indexer for a given namespace and name.
func (s sourceRepositoryNamespaceLister) Get(name string) (*v4beta1.SourceRepository, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v4beta1.Resource("sourcerepository"), name)
	}
	return obj.(*v4beta1.SourceRepository), nil
}
