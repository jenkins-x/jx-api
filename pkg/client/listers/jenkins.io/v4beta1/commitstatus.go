...
type CommitStatusLister interface {
	...
	List(selector labels.Selector) (ret []*v4beta1.CommitStatus, err error)
...
// List lists all CommitStatuses in the indexer.
func (s *commitStatusLister) List(selector labels.Selector) (ret []*v4beta1.CommitStatus, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v4beta1.CommitStatus))
	})
	return ret, err
}
...
type CommitStatusNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v4beta1.CommitStatus, error)
...
// List lists all CommitStatuses in the indexer for a given namespace.
func (s commitStatusNamespaceLister) List(selector labels.Selector) (ret []*v4beta1.CommitStatus, err error) {
...
// Get retrieves the CommitStatus from the indexer for a given namespace and name.
func (s commitStatusNamespaceLister) Get(name string) (*v4beta1.CommitStatus, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v4beta1.Resource("commitstatus"), name)
	}
	return obj.(*v4beta1.CommitStatus), nil
}
