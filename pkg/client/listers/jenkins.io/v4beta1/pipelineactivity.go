...
type PipelineActivityLister interface {
	...
	List(selector labels.Selector) (ret []*v4beta1.PipelineActivity, err error)
...
// List lists all PipelineActivities in the indexer.
func (s *pipelineActivityLister) List(selector labels.Selector) (ret []*v4beta1.PipelineActivity, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v4beta1.PipelineActivity))
	})
	return ret, err
}
...
type PipelineActivityNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v4beta1.PipelineActivity, error)
...
// List lists all PipelineActivities in the indexer for a given namespace.
func (s pipelineActivityNamespaceLister) List(selector labels.Selector) (ret []*v4beta1.PipelineActivity, err error) {
...
// Get retrieves the PipelineActivity from the indexer for a given namespace and name.
func (s pipelineActivityNamespaceLister) Get(name string) (*v4beta1.PipelineActivity, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v4beta1.Resource("pipelineactivity"), name)
	}
	return obj.(*v4beta1.PipelineActivity), nil
}
