...
type TeamLister interface {
	...
	List(selector labels.Selector) (ret []*v4beta1.Team, err error)
...
// List lists all Teams in the indexer.
func (s *teamLister) List(selector labels.Selector) (ret []*v4beta1.Team, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v4beta1.Team))
	})
	return ret, err
}
...
type TeamNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v4beta1.Team, error)
...
// List lists all Teams in the indexer for a given namespace.
func (s teamNamespaceLister) List(selector labels.Selector) (ret []*v4beta1.Team, err error) {
...
// Get retrieves the Team from the indexer for a given namespace and name.
func (s teamNamespaceLister) Get(name string) (*v4beta1.Team, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v4beta1.Resource("team"), name)
	}
	return obj.(*v4beta1.Team), nil
}
