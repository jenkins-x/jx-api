...
type ExtensionLister interface {
	...
	List(selector labels.Selector) (ret []*v4beta1.Extension, err error)
...
// List lists all Extensions in the indexer.
func (s *extensionLister) List(selector labels.Selector) (ret []*v4beta1.Extension, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v4beta1.Extension))
	})
	return ret, err
}
...
type ExtensionNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v4beta1.Extension, error)
...
// List lists all Extensions in the indexer for a given namespace.
func (s extensionNamespaceLister) List(selector labels.Selector) (ret []*v4beta1.Extension, err error) {
...
// Get retrieves the Extension from the indexer for a given namespace and name.
func (s extensionNamespaceLister) Get(name string) (*v4beta1.Extension, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v4beta1.Resource("extension"), name)
	}
	return obj.(*v4beta1.Extension), nil
}
