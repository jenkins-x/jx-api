...
type EnvironmentRoleBindingLister interface {
	...
	List(selector labels.Selector) (ret []*v4beta1.EnvironmentRoleBinding, err error)
...
// List lists all EnvironmentRoleBindings in the indexer.
func (s *environmentRoleBindingLister) List(selector labels.Selector) (ret []*v4beta1.EnvironmentRoleBinding, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v4beta1.EnvironmentRoleBinding))
	})
	return ret, err
}
...
type EnvironmentRoleBindingNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v4beta1.EnvironmentRoleBinding, error)
...
// List lists all EnvironmentRoleBindings in the indexer for a given namespace.
func (s environmentRoleBindingNamespaceLister) List(selector labels.Selector) (ret []*v4beta1.EnvironmentRoleBinding, err error) {
...
// Get retrieves the EnvironmentRoleBinding from the indexer for a given namespace and name.
func (s environmentRoleBindingNamespaceLister) Get(name string) (*v4beta1.EnvironmentRoleBinding, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v4beta1.Resource("environmentrolebinding"), name)
	}
	return obj.(*v4beta1.EnvironmentRoleBinding), nil
}
